...
`,
  ...
  it('queues a pasted video (file:// part) while a turn is streaming', async () => {
    ...
    try {
      ...
      expect(session.prompt).toHaveBeenCalledWith(parts, { agentId: 'main' });
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
  ...
  it('steers fresh input while a goal is active even when the streaming phase is idle', async () => {
    ...
    expect(session.steer).toHaveBeenCalledWith([{ type: 'text', text: 'hello mid-goal' }]);
    expect(session.prompt).not.toHaveBeenCalled();
    expect(driver.state.transcriptEntries).toEqual([
      expect.objectContaining({
        kind: 'user',
  ...
  it('steers a queued message at a turn boundary while a goal is active', async () => {
    ...
    await vi.waitFor(() => {
      expect(session.steer).toHaveBeenCalledWith([{ type: 'text', text: 'mid-goal note' }]);
    });
    expect(session.prompt).not.toHaveBeenCalled();
    ...
    driver.state.queuedMessages = [
      {
        ...
        parts: [
          { type: 'text', text: 'look ' },
          { type: 'image', source: { kind: 'url', url: 'data:image/png;base64,qrs=' } },
        ],
        imageAttachmentIds: [attachment.id],
      },
    ];
  ...
  it('steers plain text followed by a media item with a blank-line separator', async () => {
    ...
    driver.state.queuedMessages = [
      { text: 'hello', agentId: 'main' },
      {
        ...
        agentId: 'main',
        parts: [{ type: 'image', source: { kind: 'url', url: 'data:image/png;base64,qrs=' } }],
        imageAttachmentIds: [attachment.id],
      },
    ];
