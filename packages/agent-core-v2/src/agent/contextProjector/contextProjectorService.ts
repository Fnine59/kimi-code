...
export class AgentContextProjectorService implements IAgentContextProjectorService {
  ...
  projectMediaDegraded(
    ...
    return degradeOlderMediaParts(
      this.projectWithTrace(this.historyForProjection(messages, options), project),
  ...
  projectMediaStripped(
    ...
    const projected = this.projectWithTrace(this.historyForProjection(messages, options), project);
