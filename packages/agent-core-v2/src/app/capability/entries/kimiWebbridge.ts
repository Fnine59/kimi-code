 ...
 *
 * The plugin zip follows the region `cdnBase` via `ctx.resolveRegion`
 * (falling back to env override > install marker > cn default without the
 * hook); the daemon binary is a content-CDN artifact and funnels through
 * `kimiCdnContentUrl`.
 */
