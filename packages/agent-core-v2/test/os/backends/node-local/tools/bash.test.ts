 ...
 * builds minimal fake runtime / `IHostProcessService`, `IKaos`, and
 ...
 *
 * Deviations from v1:
 *   - v1's `execWithEnv(args, env)` is now `runner.spawn(command, args, options)`,
 *     so spawn-call assertions read `options.env` from the third argument.
 */
