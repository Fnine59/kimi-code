...
describe('Agent loop', () => {
  ...
  it('runs a text-only agent turn from prompt to completion', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] agent.status.updated        { "contextTokens": 3, "rawContextTokens": 3 }
      ...
      [emit] agent.status.updated        { "contextTokens": 13, "rawContextTokens": 16 }
      [emit] turn.step.completed         { "turnId": 0, "step": 1, "stepId": "<uuid-1>", "usage": { "inputOther": 3, "output": 10, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "end_turn", "providerFinishReason": "completed", "rawFinishReason": "stop" }
      [emit] agent.activity.updated      { "lifecycle": "ready", "turn": { "turnId": 0, "origin": { "kind": "user" }, "phase": "running", "step": 1, "ending": false, "pendingApprovals": [], "activeToolCalls": [], "since": "<time>" }, "background": [] }
  ...
  it('fails the turn after a filtered step completes', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] agent.status.updated        { "contextTokens": 8, "rawContextTokens": 11 }
  ...
  it('runs an agent turn through registered tool approval and execution', async () => {
    ...
    expect(await ctx.untilApproval(true)).toMatchInlineSnapshot(`
      ...
      [emit] agent.status.updated            { "contextTokens": 26, "rawContextTokens": 29 }
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] turn.step.completed                 { "turnId": 0, "step": 1, "stepId": "<uuid-1>", "usage": { "inputOther": 5, "output": 21, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "tool_use", "providerFinishReason": "tool_calls", "rawFinishReason": "tool_calls" }
      ...
      [emit] agent.status.updated                { "contextTokens": 44, "rawContextTokens": 47 }
